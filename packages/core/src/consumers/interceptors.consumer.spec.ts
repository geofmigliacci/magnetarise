...
describe('InterceptorsConsumer', () => {
  ...
  beforeAll(() => {
    MagnetariseContainer.interceptors.add(MultiplyByTwo);
    MagnetariseContainer.interceptors.add(RemoveOne);
  });
  ...
  it('should pickResult from Promise', async () => {
    const result = await InterceptorsConsumer.pickResult(
      new Promise((resolve) => {
        resolve([42]);
      })
    );
  ...
  it('should create generate interceptors', async () => {
    ...
    expect(inFnSpy).toHaveBeenCalledWith([RemoveOne, MultiplyByTwo]);

    expect(outFnSpy).toHaveBeenCalledWith([MultiplyByTwo, RemoveOne]);
