...
export class PipesConsumer {
  static pipesFn(
    pipesMetadata: Map<number, InjectionToken[]>
    ...
    return async (
      ...
      for (const [index, values] of pipesMetadata) {
        for (const pipe of values) {
          const concretePipe =
            MagnetariseContainer.container.resolve<PipeTransform>(pipe);
          args[index] = await this.pickResult(
            concretePipe.transform(
              executionContext,
              executionContext.getArgByIndex(index)
            )
          );
        }
      }
  ...
  static async pickResult<T>(result: T | Promise<T>): Promise<T> {
    return result;
  }
}
